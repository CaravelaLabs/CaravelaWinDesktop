...
if (IS_WINDOWS) {
  ...
  // expose Caravela sessions/config to every packaged app on the machine.
...
// HERMES_HOME — the user-facing root for everything Caravela-related. Mirrors
...
// active-profile.json records which Caravela profile the desktop launches its
...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || 'Caravela'
...
// electron-builder derives from the build `appId` (com.caravela.app) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.caravela.app')
}

// Seed the native About panel with the live Caravela version. This is refreshed
...
let bootProgressState = {
  ...
  message: 'Waiting to start Caravela backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Caravela will start automatically when it completes…',
        12
      )
    },
...
function findSystemPython() {
  ...
  // CPython but several Caravela deps (notably pywinpty's Rust-built
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  try {
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Caravela backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}
...
// `hermes update`). Instead we spawn the staged Caravela-Setup binary with
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Caravela — this window will close and the updater will open. Don’t reopen Caravela yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    if (!lock.unlocked) {
      ...
      const message =
        'Update aborted: another process is holding the Caravela install open ' +
        '(a second Caravela window or a terminal running hermes?). Close it and retry.'
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Caravela will keep running — try again, or run \`hermes update\` from a terminal.`
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Caravela — this window will close. Don’t reopen Caravela yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
function resolveHermesBackend(backendArgs) {
  ...
  return {
    ...
    label: 'Caravela not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Caravela install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Caravela recovery was handed off to Caravela Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Caravela install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Caravela bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
  ...
  if (!isHermesSourceRoot(ACTIVE_HERMES_ROOT)) {
    throw new Error(
      `Caravela install at ${ACTIVE_HERMES_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Caravela' terminal tool calls bash.exe
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Caravela on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Caravela.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `Caravela venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }
  ...
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Caravela runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Caravela backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Caravela backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Caravela backend after ${timeoutMs}ms`))
        })
...
// signal so the chat composer doesn't stay stuck on "Starting Caravela...".
...
//
// Hosted Caravela gateways gate the dashboard behind an OAuth provider (e.g.
...
// single-shot boot() with no retry) surfaced as the "Caravela couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Caravela
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Hermes Cloud agent…' : 'Sign in to Caravela gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Caravela backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// so a single override flips every Caravela surface to the same portal.
...
// authenticates via PRIVY, not the Caravela gateway session cookies, so this
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Caravela gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Caravela gateway is selected, but no session token is saved. ' +
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Caravela
...
async function testDesktopConnectionConfig(input: any = {}) {
        ...
        try {
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Caravela on the remote host before connecting with Desktop SSH.'
            }
          }
  ...
  // connect to Caravela gateway". Mirror the renderer's connect here so the test
...
async function ensureBackend(profile) {
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Caravela backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Caravela backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  rememberLog(`Starting Caravela backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Caravela backend for profile "${profile}" failed to start: ${error.message}`)
  ...
  child.once('exit', (code, signal) => {
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Caravela backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    ...
    label: `Caravela backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Caravela backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
async function startHermes() {
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Caravela backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Caravela backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Caravela backend at ${remote.baseUrl}`, 24)
      await waitForHermes(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Caravela backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Caravela runtime', 28)
    ...
    await advanceBootProgress('backend.spawn', `Starting Caravela backend via ${backend.label}`, 84)
    rememberLog(`Starting Caravela backend via ${backend.label}`)
    ...
    hermesProcess.once('error', error => {
      ...
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        ...
        rejectBackendStart?.(new Error('Caravela backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Caravela backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Caravela backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      ...
      if (!backendReady) {
        const message = `Caravela backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Caravela backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentHermesLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Caravela backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Caravela backend to become ready', 90)
    ...
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Caravela backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Caravela is
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Caravela app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Caravela icon drops out of cmd/alt-tab — especially when the
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Caravela in the normal
...
// "Starting Caravela…". Before the renderer's backoff loop reconnects, it asks us
...
ipcMain.handle('hermes:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    if (!backend.command) {
      return { ok: false, error: 'Caravela is not installed yet' }
    }
...
// Resolve the canonical Caravela version (the one `release.py` bumps in
// hermes_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Caravela version instead of the Electron app's own package.json version,
...
// Re-resolve the live Caravela version and push it into the native About panel
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Caravela agent venv at ${VENV_ROOT}.`
    }
  }
