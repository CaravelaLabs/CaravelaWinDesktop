...
/// Spawn the locally-built Caravela desktop binary, then close the installer
...
pub async fn launch_hermes_desktop(
    ...
    let exe_path = resolve_hermes_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Caravela desktop at {}. The desktop build step \
    ...
    tracing::info!(?exe_path, "launching Caravela desktop");
...
pub(crate) fn resolve_hermes_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Caravela.exe"),
            ("win-arm64-unpacked", "Caravela.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Caravela.app/Contents/MacOS", "Caravela"),
            ("mac-arm64/Caravela.app/Contents/MacOS", "Caravela"),
        ]
    } else {
...
pub(crate) fn resolve_hermes_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        // .../Caravela.app/Contents/MacOS/Hermes -> .../Caravela.app
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_hermes_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Caravela desktop app")
    })?;
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Caravela"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Caravela.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Caravela.exe");
    ...
    // what the updater ditto's over /Applications/Caravela.app). A regression in
