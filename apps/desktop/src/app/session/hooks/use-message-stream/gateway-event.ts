 ...
 * billing-specific toast — never the generic "Caravela error" — with a smart CTA
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        // A runtime skin switch (Caravela activating an authored skin, or `/skin`
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'Caravela reported an error',
        ...
        // native OS notification when Caravela is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'Caravela reported an error'
