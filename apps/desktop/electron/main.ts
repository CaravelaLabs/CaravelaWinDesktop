...
async function applyUpdatesPosixInApp(opts: any) {
  ...
  // Put the Caravela-managed Node and the venv on PATH so `hermes desktop`'s
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Caravela (git + dependencies)…', percent: 10 })
  ...
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Caravela to retry.',
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Caravela…', percent: 100 })
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Caravela to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Caravela desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Caravela to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Caravela.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Caravela.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Caravela to load the new version.',
      percent: 100
    })
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Caravela to load the new version.',
      percent: 100
    })
    ...
    const req = client.request(
      parsed,
      {
        ...
        headers: {
          ...
          'X-Caravela-Session-Token': token,
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Caravela-Session-Token`` header — used by the auth-mode probe before
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    if (
      ...
      const err = new Error(
        'Remote Caravela gateway uses OAuth, but you are not signed in. ' +
    ...
    if (!rawEnvToken) {
      throw new Error(
        'HERMES_DESKTOP_REMOTE_URL is set but HERMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Caravela backend.'
      )
    }
...
async function spawnPoolBackend(profile, entry) {
  ...
  child.once('exit', (code, signal) => {
    ...
    if (!ready) {
      rejectStart?.(
        new Error(`Caravela backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
...
async function startHermes() {
  ...
  const connectionPromise = (async () => {
    ...
    hermesProcess.once('exit', (code, signal) => {
      ...
      if (!backendReady) {
        const message = `Caravela backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Caravela' TUI into
